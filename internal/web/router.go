...
func (s *Server) Handler() http.Handler {
	...
	mux.HandleFunc("POST /api/sharing/ftp/adopt", s.requireAuth(s.handleSharingFTPAdopt))
	mux.HandleFunc("POST /api/sharing/ftp/unadopt", s.requireAuth(s.handleSharingFTPUnadopt))
	mux.HandleFunc("POST /api/sharing/ftp/settings", s.requireAuth(s.handleSharingFTPSettings))
	mux.HandleFunc("POST /api/sharing/ftp/service", s.requireAuth(s.handleSharingFTPService))
	mux.HandleFunc("POST /api/sharing/accounts/{name}/ftp", s.requireAuth(s.handleSharingFTPAccess))
	mux.HandleFunc("POST /api/sharing/accounts/{name}/ftp/clear", s.requireAuth(s.handleSharingFTPClear))
