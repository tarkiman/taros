...
`,[T(`border`,`
 ...
 border: var(--n-border);
 pointer-events: none;
 `),y(`closable`,[v(`alert-body`,[T(`title`,`
 padding-right: 24px;
 `)])]),T(`icon`,{color:`var(--n-icon-color)`}),v(`alert-body`,{padding:`var(--n-padding)`},[T(`title`,{color:`var(--n-title-text-color)`}),T(`content`,{color:`var(--n-content-text-color)`})]),h({originalTransition:`transform .3s var(--n-bezier)`,enterToProps:{transform:`scale(1)`},leaveToProps:{transform:`scale(0.9)`}}),T(`icon`,`
 position: absolute;
 left: 0;
 top: 0;
 align-items: center;
