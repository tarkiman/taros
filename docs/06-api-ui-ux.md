...
| `POST /api/files/op` | Body JSON: `{action, path, newPath?, paths?}`. `mkdir\|create\|rename\|delete` sinkron (instan). `rename` hanya mengganti nama di folder yang sama dan **tidak pernah menimpa** (`renameat2 RENAME_NOREPLACE`); nama tidak valid → 400 `file_name_invalid`, nama terpakai → 409 `file_exists`, tidak ada → 404 `file_not_found`, tanpa izin → 403 `file_permission_denied`, read-only → 403 `file_read_only`, penuh → 507 `file_no_space`. `copy\|cut` cuma menyimpan `paths` ke clipboard sesi (`auth.Session`), tidak ada I/O. `paste` **selalu** mengembalikan `{jobId}` — bahkan kalau semua entry ternyata instant-rename, satu jalur kode klien untuk semua kasus |
...
| `POST /api/settings/password` | Body `{currentPassword, newPassword}` → `{ok, otherSessionsClosed}`. Ganti password akun sendiri: 403 `wrong_password` bila password saat ini salah, 429 bila rate limiter login penuh, 400 `password_too_short`/`password_too_long`/`password_same_as_old`. Semua sesi lain akun itu berakhir, sesi ini tetap |
| `POST /api/settings/users/{username}/password` | Body `{newPassword, password}` (`password` = password akun yang meminta) → `{ok, sessionsClosed}`. Reset password akun lain; semua sesi akun itu berakhir (bila targetnya diri sendiri, sesi ini tetap). 404 `user_not_found`, rate limiter yang sama |
