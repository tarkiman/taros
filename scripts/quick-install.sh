...
if [[ "$IS_DARWIN" -eq 1 ]]; then
  ...
  if [[ ! -f "$DEST/config.yaml" ]]; then
    ...
    # config.example.yaml hardcodes /etc/taros/credentials.yaml and
    # /opt/taros/quick-links.yaml — correct for the Linux systemd path
    # (install.sh creates/chowns those directories), but there's no such
    # setup here and no sudo/root to make one. Point both at this install's
    # own directory instead, same sed-based customization pattern
    # install.sh already uses for listen/rootDir.
    sed -i '' "s|^  credentialsFile:.*|  credentialsFile: \"$DEST/credentials.yaml\"|" "$DEST/config.yaml"
    sed -i '' "s|^  quickLinksFile:.*|  quickLinksFile: \"$DEST/quick-links.yaml\"|" "$DEST/config.yaml"
  fi
